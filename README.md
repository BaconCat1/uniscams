...
`api.js`: Data fetching, caching, and server registry

`ui.js`: Card rendering and archive tab

`graphs.js`: Stats tab charts

`app.js`: Bootstrap pipeline, search, and lifecycle

`data.json`: Scammer UUIDs, alt maps, Discord links, and server tags

`discord.json`: Fallback Discord user data

`EXTRASOURCES.md`: Supplementary sources for various data points that don't fit anywhere else

`tools/admin.html`: GUI to import, append new data, and export `data.json`

`tools/discord_uid.js`: Node.js command line utility to generate `discord.json` from `data.json`
